...
public class Censor {
	...
	public static void loadConfig(JagexArchive jagexArchive) {
		Packet packet = new Packet(jagexArchive.getDataForName("fragmentsenc.txt"));
		Packet secondPacket = new Packet(jagexArchive.getDataForName("badenc.txt"));
		Packet thirdPacket = new Packet(jagexArchive.getDataForName("domainenc.txt"));
		Packet fourthPacket = new Packet(jagexArchive.getDataForName("tldlist.txt"));
		Censor.readValues(packet, secondPacket, thirdPacket, fourthPacket);
	}

	private static void readValues(Packet packet, Packet stream_1, Packet stream_2, Packet stream_3) {
		Censor.readBadEnc(stream_1);
		Censor.readDomainEnc(stream_2);
		Censor.readFragmentsEnc(packet);
		Censor.readTldList(stream_3);
	}

	private static void readTldList(Packet packet) {
		int i = packet.getInt();
		Censor.tldList = new char[i][];
		Censor.tldArray = new int[i];
		for (int j = 0; j < i; j++) {
			Censor.tldArray[j] = packet.getUnsignedByte();
			char ac[] = new char[packet.getUnsignedByte()];
			for (int k = 0; k < ac.length; k++) {
				ac[k] = (char) packet.getUnsignedByte();
			}
			Censor.tldList[j] = ac;
		}
	}

	private static void readBadEnc(Packet packet) {
		int j = packet.getInt();
		Censor.badenc = new char[j][];
		Censor.badencByte = new byte[j][][];
		Censor.initBadenc(packet, Censor.badenc, Censor.badencByte);
	}

	private static void readDomainEnc(Packet packet) {
		int i = packet.getInt();
		Censor.domainenc = new char[i][];
		Censor.initDomainenc(Censor.domainenc, packet);
	}

	private static void readFragmentsEnc(Packet packet) {
		Censor.fragmentsenc = new int[packet.getInt()];
		for (int i = 0; i < Censor.fragmentsenc.length; i++) {
			Censor.fragmentsenc[i] = packet.getUnsignedShort();
		}
	}

	private static void initBadenc(Packet packet, char ac[][], byte abyte0[][][]) {
		for (int j = 0; j < ac.length; j++) {
			char ac1[] = new char[packet.getUnsignedByte()];
			for (int k = 0; k < ac1.length; k++) {
				ac1[k] = (char) packet.getUnsignedByte();
			}
			ac[j] = ac1;
			byte abyte1[][] = new byte[packet.getUnsignedByte()][2];
			for (int l = 0; l < abyte1.length; l++) {
				abyte1[l][0] = (byte) packet.getUnsignedByte();
				abyte1[l][1] = (byte) packet.getUnsignedByte();
			}
			if (abyte1.length > 0) {
				abyte0[j] = abyte1;
			}
	...
	private static void initDomainenc(char[][] ac, Packet packet) {
		for (int j = 0; j < ac.length; j++) {
			char ac1[] = new char[packet.getUnsignedByte()];
			for (int k = 0; k < ac1.length; k++) {
				ac1[k] = (char) packet.getUnsignedByte();
	...
	private static void processBadenc(char ac[]) {
		for (int i = 0; i < 2; i++) {
			for (int j = Censor.badenc.length - 1; j >= 0; j--) {
				Censor.censorStringIn(Censor.badencByte[j], ac, Censor.badenc[j]);
			}
		}
	}
	...
	private static void removeEmails(char ac[]) {
		...
		for (int i = Censor.domainenc.length - 1; i >= 0; i--) {
			Censor.removeEmailFrom(ac, Censor.domainenc[i], ac3, ac1);
		}
	}
	...
	private static void removeEmailFrom(char ac[], char ac1[], char ac2[], char ac3[]) {
		...
		for (int k = 0; k <= (ac.length - ac1.length); k += j) {
			...
			while (l < ac.length) {
				...
				if ((j1 = Censor.checkForCharBasicLeetSpeak(c, ac1[i1 - 1], c1)) > 0) {
					l += j1;
					if (i1 == 1) {
						j++;
					}
				...
				if ((i1 >= ac1.length) || !Censor.isNotAlphanumeric(c)) {
					break;
				}
				l++;
			}
	...
	private static int isAtSignBeforeDomain(char ac[], char ac1[], int j) {
		...
		for (int k = j - 1; k >= 0; k--) {
			if (!Censor.isNotAlphanumeric(ac[k])) {
				break;
			}
			if (ac[k] == '@') {
				return 3;
		...
		for (int i1 = j - 1; i1 >= 0; i1--) {
			if (!Censor.isNotAlphanumeric(ac1[i1])) {
				break;
			}
			if (ac1[i1] == '*') {
				l++;
	...
	private static int isDotAfterDomain(char ac[], int i, char ac1[]) {
		...
		for (int j = i + 1; j < ac1.length; j++) {
			if (!Censor.isNotAlphanumeric(ac1[j])) {
				break;
			}
			if ((ac1[j] == '.') || (ac1[j] == ',')) {
				return 3;
		...
		for (int l = i + 1; l < ac1.length; l++) {
			if (!Censor.isNotAlphanumeric(ac[l])) {
				break;
			}
			if (ac[l] == '*') {
				k++;
	...
	private static void removeTlds(char[] ac) {
		...
		Censor.censorStringIn(null, ac3, ac4);
		for (int i = 0; i < Censor.tldList.length; i++) {
			Censor.removeTldFrom(ac3, Censor.tldList[i], Censor.tldArray[i], ac1, ac);
		}
	}
				...
				if (flag1) {
					...
					if (k1 > 2) {
						...
						for (int i3 = i2 - 1; i3 >= 0; i3--) {
							if (flag3) {
								if (Censor.isNotAlphanumeric(ac3[i3])) {
									break;
								}
								i2 = i3;
					...
					if (l1 > 2) {
						...
						for (int k3 = j2 + 1; k3 < ac3.length; k3++) {
							if (flag5) {
								if (Censor.isNotAlphanumeric(ac3[k3])) {
									break;
								}
								j2 = k3;
	...
	private static int isSlashAfterTld(char ac[], char ac1[], int i) {
		...
		for (int j = i + 1; j < ac.length; j++) {
			if (!Censor.isNotAlphanumeric(ac[j])) {
				break;
			}
			if ((ac[j] == '\\') || (ac[j] == '/')) {
				return 3;
		...
		for (int l = i + 1; l < ac.length; l++) {
			if (!Censor.isNotAlphanumeric(ac1[l])) {
				break;
			}
			if (ac1[l] == '*') {
				k++;
	...
	private static void censorStringIn(byte[][] abyte0, char[] ac, char[] ac1) {
		...
		for (int k = 0; k <= (ac.length - ac1.length); k += j) {
			...
			while ((l < ac.length) && (!flag2 || !flag3)) {
				...
				if ((k1 = Censor.checkForCharAdvancedLeetspeak(c2, c, ac1[i1 - 1])) > 0) {
					l += k1;
					if (i1 == 1) {
						j++;
					}
				...
				if ((i1 >= ac1.length) || !Censor.isDigitOrSymbol(c)) {
					break;
				}
			...
			if ((i1 >= ac1.length) && (!flag2 || !flag3)) {
				...
				if (!flag1) {
					...
					byte byte0 = Censor.getCharCodeOfSomeSort(c1);
					byte byte1 = Censor.getCharCodeOfSomeSort(c3);
					...
					if (!flag5 || !flag6) {
						...
						for (; !flag7 && (k2 < l); k2++) {
							if ((k2 >= 0) && (!Censor.isNotAlphanumeric(ac[k2]) || (ac[k2] == '\''))) {
								...
								for(j3 = 0; j3 < 3; j3++)
								{
									if (((k2 + j3) >= ac.length) || (Censor.isNotAlphanumeric(ac[k2 + j3]) && (ac[k2 + j3] != '\''))) {
										break;
									}
									ac2[j3] = ac[k2 + j3];
								}
	...
	private static byte getCharCodeOfSomeSort(char c)
	{
		if((c >= 'a') && (c <= 'z')) {
			return (byte)((c - 97) + 1);
		}
	...
	private static void censorLongNumbers(char ac[])
	{
		int j;
		int k = 0;
		int l = 0;
	...
	private static boolean charsMatchFragment(char ac[]) {
		...
		int k = 0;
		int l = Censor.fragmentsenc.length - 1;
		if((j == Censor.fragmentsenc[k]) || (j == Censor.fragmentsenc[l])) {
			return true;
		}
		do {
			int i1 = (k + l) / 2;
			if(j == Censor.fragmentsenc[i1]) {
				return true;
			}
			if(j < Censor.fragmentsenc[i1]) {
				l = i1;
	...
	private static int hashCode(char ac[]) {
		if(ac.length > 6) {
			return 0;
		}
		int k = 0;

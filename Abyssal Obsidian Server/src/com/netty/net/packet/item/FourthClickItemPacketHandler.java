 ...
 * @since Sep 21, 2011 12:25:04 PM
 */
public class FourthClickItemPacketHandler implements PacketHandler {
	...
	public void handlePacket(PacketAttribute packetAttribute, Player player) {
		...
		switch (interfaceID) {
			...
			case 7423:
				if ((slot >= 0) && (slot < 28)) {
					player.getBank().depositItem(player, slot, new Item(id, player.getInventory().getInventoryContainer().getAmount(item)));
				}
				break;
		}
	}
